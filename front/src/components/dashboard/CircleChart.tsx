...
const CircleChart = ({ title, percentage, isProperty }: CircleChartProps) => {
  return (
    <div className="flex h-auto w-full flex-col items-center gap-6 rounded-lg bg-white p-4 text-white shadow-[0_4px_8px_0_rgba(0,0,0,0.1)] md:h-[358px] md:gap-10 md:p-8">
      <div className="flex h-[42px] w-full flex-col gap-5">
        <div
          className={`flex h-[22px] w-full flex-row ${!isProperty ? 'justify-between' : 'items-center justify-center'}`}>
        ...
        {!isProperty && <div className="border-secondary-content/40 border-2"></div>}
      </div>
